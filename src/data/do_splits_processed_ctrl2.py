    ...
    for question in split_gen:
        ...
        for elem in split_ctrl:
            if sections_uuids_concat == elem['sections_uuids_concat']:
                # append to current elem if it belongs to the same <section,skill> pair
...
if __name__ == '__main__':
    ...
    Path('../../data/FairyTaleQA_Dataset/processed_ctrl2').mkdir(parents=True, exist_ok=True)
    
    # save faitytaleqa processed splits to json files
    with open('../../data/FairyTaleQA_Dataset/processed_ctrl2/train.json', 'w', encoding='utf-8') as fout:
        json.dump(train_ctrl , fout)

    with open('../../data/FairyTaleQA_Dataset/processed_ctrl2/val.json', 'w', encoding='utf-8') as fout:
        json.dump(val_ctrl , fout)

    with open('../../data/FairyTaleQA_Dataset/processed_ctrl2/test.json', 'w', encoding='utf-8') as fout:
        json.dump(test_ctrl , fout)

    print("CTRL2 splits have been successfully created.")